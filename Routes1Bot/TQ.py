...
table = pd.read_excel('BaseYandex.xlsx')
...
class YandexDiskUpdater:
    def __init__(self, public_link, save_path="BaseYandex.xlsx"):
        self.public_link = public_link
        self.save_path = save_path
        self.last_hash = None

    def download_file(self):
        """Скачивает или обновляет файл с Яндекс.Диска"""
        try:
            ...
            print(f"[{datetime.now()}] Файл успешно загружен/обновлён: {os.path.abspath(self.save_path)}")
            return True
...
def callback_start(call):
    ...
    #Обновление таблицы
    updater = YandexDiskUpdater(public_link="https://disk.yandex.ru/i/xQPNW_-SjGvwOQ")
    if not updater.download_file():
...
def step1(message):
    ...
    # Читаем Excel
    df = pd.read_excel('BaseYandex.xlsx')  #укажи путь к файлу
    #excel_data = pd.read_excel('BaseYandex.xlsx').replace({pd.NA: None}).to_dict('table') 

    # Выводим данные
    first_value = df.iloc[1, ] #можно выбрать [строку, столбец]
    bot.send_message(message.chat.id, str(first_value))
